...
import orbisoftware.hla_tools.spectrum_hla_monitor.hla_sender.PublishSolarSystem;
import orbisoftware.hla_tools.spectrum_hla_monitor.hla_sender.PublishPlanetHasCompletedAnOrbit;
...
public class SampleViewerGUI implements ActionListener {
   ...
   private void createCommonControlPanels(JPanel nodeStatePanel,
      ...
      autoUpdateCheckBox.addItemListener(new ItemListener() {
         public void itemStateChanged(ItemEvent e) {
            boolean selected = (e.getStateChange() == ItemEvent.SELECTED);
            if (selected) {
               ReceivePlanetHasCompletedAnOrbit.getInstance().autoUpdateEnabled = true;
               ReceiveSolarSystem.getInstance().autoUpdateEnabled = true;
            }
            else {
               ReceivePlanetHasCompletedAnOrbit.getInstance().autoUpdateEnabled = false;
               ReceiveSolarSystem.getInstance().autoUpdateEnabled = false;
            }
         }
      });
   ...
   public void shutdownReq() {
      ...
      while ( !hlaTopic.readEventHandlerShutdownComplete
           ...
           //|| !orbisoftware.hla_receiver.HLAFederateThread.shutdownComplete
           //|| !orbisoftware.hla_sender.HLAFederateThread.shutdownComplete) {
         try {
            Thread.sleep(200);
   ...
   public void actionPerformed(ActionEvent event) {
      ...
      if ((JButton) event.getSource() == publishButton) {
        ...
        if (hlaTopic.name.equals("SolarSystem")) {
           ...
           PublishSolarSystem.getInstance().sideLoadPublishSample(objectRef);
        }
        
        if (hlaTopic.name.equals("PlanetHasCompletedAnOrbit")) {
           
           Object objectRef = symbolRowModel.getOricSymbolMap().symMapToObj();
           PublishPlanetHasCompletedAnOrbit.getInstance().sideLoadPublishSample(objectRef);
        }
         ...
         try {
            hlaObjectClass = Class.forName(hlaTopic.dataType);
